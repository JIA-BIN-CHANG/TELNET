...
with open('./config_clean.json','r') as f:
    config = json.load(f)
...
#     print(f'load keyshot video name: {video_name}')
    yang_keyshot_dir = "D:/yang_bbc_keyshot/keyshot"
    ...
    for pred_scene in scenes:
        if pred_scene[-1]>tmp_point:
            if(pred_scene[-1] == total_shot):
                continue
#             print('pred_scene[-1]: ',pred_scene[-1])
...
def evaluate_window(label_dir,model,video_list,mask,windowSize,ground_dir,bbc=False):
    ...
    for i in range(len(video_list)):                                            ## 因為每個Dataset都有很多影片，故要計算每個的平均
...
###############################################yang loss
        ...
        # print(f'all_link_np shape: {all_link_np.shape}')
